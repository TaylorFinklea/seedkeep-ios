...
`main` (TestFlight 0.4.0 build 52; Production deletion Gates 1–2 passed).
...
- [ ] Production participant and solo-owner deletion gates passed on build 52. Gate 1 exposed stale Better Auth cookie rejection on immediate re-sign-in; seedkeep-server `6742676` now expires auth cookies and Gate 2 proved immediate re-sign-in works without reinstalling and restores no deleted data. Next: two-account transfer gate only.

## Blockers

- Gate 3 requires two physical devices and two distinct disposable Apple identities. Do not enroll the user's wife as a destructive test subject; keep `seedkeep-27d.12` open until safe identities are available.
