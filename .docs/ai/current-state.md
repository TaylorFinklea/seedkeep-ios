...
- [ ] Next: wait for Apple CDN AASA propagation and build-52 processing, then run authenticated disposable-account smoke plus participant/solo-owner/two-account Production gates in `.docs/ai/device-verify.md`.

## Blockers

- Apple CDN still serves the pre-handoff AASA; do not assign/install build 52 or begin destructive device gates until `/garden-handoff/*` propagates.
